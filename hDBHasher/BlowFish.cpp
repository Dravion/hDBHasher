...
namespace HM
{
   BlowFishEncryptor::BlowFishEncryptor ()
   {
      ...
      std::string sKey = NOT_SECRET_KYE;
      int iKeyLen = sKey.length();
      BYTE *buf = new BYTE[iKeyLen + 1];
      memset(buf, 0, iKeyLen + 1);
      strncpy_s((char*) buf, iKeyLen+1, sKey.c_str(), iKeyLen);
   ...
   std::string  BlowFishEncryptor::ToHex_(BYTE *Buf, int iBufLen)
   {
      std::string sRetVal;
   ...
   int BlowFishEncryptor::ToByteArray_(const std::string &sHex, BYTE *OutArray)
   {
   ...
   std::string
   BlowFishEncryptor::EncryptToString(const std::string &sUnEncrypted)
   {

      DWORD InLength = sUnEncrypted.length();

      const int bufSize = 255;
      BYTE *pBuffer = new BYTE[bufSize];
      memset(pBuffer, 0, bufSize);
      strncpy_s((char*)pBuffer, bufSize, sUnEncrypted.c_str(), InLength);

      DWORD Length = Encode(pBuffer, pBuffer, InLength);
      
      std::string sRetVal = ToHex_(pBuffer, Length);
